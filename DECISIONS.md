...
## 2026-07-07 · +20 языков интерфейса: топ App Store, только LTR, RTL отложен
- **Решение:** добавили 20 языков в `Language`/`L10n` (было en/ru/uk → стало 23): es, zh-Hans, zh-Hant, ja, ko, de, fr, pt-BR, it, tr, nl, pl, sv, da, id, th, hi, vi, cs, el. Оставили существующую архитектуру `L10n` (плоский struct с labeled memberwise-init на ~76 полей, один `static let` на язык + `make(for:)` switch) - НЕ мигрировали на `.xcstrings`. Перевод сгенерён 20 параллельными агентами → строгий JSON per-язык → детерминированная генерация Swift из шаблона (лейблы/порядок полей не могут разъехаться, компилятор форсит полноту). Технические токены (RT60/RMS/EQ/dB/Hz/A4/OK/"Audio Spectrum Pro") оставлены непереведёнными.
- **Отвергли:** (а) включить арабский/иврит/фарси (топ по носителям) - RTL требует прокидывания `layoutDirection` (приложение оверрайдит только строки, не системную локаль, поэтому авто-RTL не срабатывает) И спектр-графики обязаны остаться LTR (ось частоты low→high не зеркалить) - отдельная работа с тестами; отложено, не выкинуто. (б) миграция на нативный String Catalog `.xcstrings` - ключуется по системной локали, сломало бы фичу "смена языка внутри приложения без смены языка устройства"; текущий struct-подход для in-app-переключения корректнее. (в) переводить самому инлайном - хуже нативность на th/hi/vi/zh/ja/ko/el и выше риск ошибок в порядке полей.
- **Почему:** LTR-топ покрывает основные App Store рынки без RTL-осложнений; labeled memberwise-init = fail-fast (пропущенное поле = ошибка компиляции, не рантайм); build под iPhone 17 sim прошёл (`** BUILD SUCCEEDED **`).
- **Триггер:** Pavel: "надо добавить еще 20 самых популярных языков".
- **Owner:** Claude / Pavel (выбор набора языков + LTR-only)
- **Связь:** [[appstore-publishing]], `AudioSpectrumPro/Language.swift`, `AudioSpectrumPro/L10n.swift`, `DEBT.md` (локализация листинга + v1.3)
